#!/usr/bin/env bash
# Instalador idempotente de la unit systemd `cam-counter-api`.
#
# La unit versionada (v1/systemd/cam-counter-api.service) NO contiene rutas
...
# `install_hailo_service.sh`. La unit es INDEPENDIENTE de `hailo-personas` (no la
# toca; ambas pueden coexistir hasta el cutover de PRs posteriores).
#
...
#                      (por defecto /etc/systemd/system/cam-counter-api.service).
#   NO_SYSTEMCTL=1     Fuerza el modo "sólo render" aunque exista systemctl.
set -euo pipefail

SERVICE_NAME="cam-counter-api"
...
chmod +x "$repo_root/v1/api/run_api.sh" 2>/dev/null || true
...
if [ "$have_systemctl" -eq 0 ]; then
  ...
  echo "[install_api_service] systemd no disponible; unit renderizada en: $dest" >&2
  echo "[install_api_service] repo detectado: $repo_root" >&2
  printf '%s' "$rendered"
  exit 0
fi
...
if [ -f "$unit_dest" ] && printf '%s' "$rendered" | cmp -s - "$unit_dest"; then
  echo "[install_api_service] unit ya actualizada en $unit_dest (sin cambios)" >&2
else
  printf '%s' "$rendered" > "$unit_dest"
  echo "[install_api_service] unit escrita en $unit_dest" >&2
fi

systemctl daemon-reload
systemctl enable --now "$SERVICE_NAME"
echo "[install_api_service] $SERVICE_NAME habilitado e iniciado (repo: $repo_root)" >&2
